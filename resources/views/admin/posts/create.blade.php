    ...
    <div class="col-md-8 offset-md-2">
        ...
        <form action="{{ route('admin.posts.store') }}" method="POST" id="post-form">
            ...
            <div class="form-group" id="user-select" style="display: none;">
                ...
                <input type="text" id="user-search" class="form-control" placeholder="Search User by Email" autocomplete="off">
                <select id="user-dropdown" class="form-control" style="display: none;"></select>
                <input type="hidden" name="user_email" id="user-email"> <!-- Hidden input for user_email -->
                <small id="no-results" class="text-danger" style="display: none;">No users found with that email.</small>
            </div>
            <br>
...
<!-- Embed the JavaScript directly into the Blade template -->
...
<script>
    ...
    $('#user-search').on('keyup', function() {
        ...
        console.log('Searching for:', query, 'with role:', role);
    ...
    $('#user-dropdown').on('change', function() {
        ...
        if (selectedUserEmail === "") { // If the "Select User from here..." option is selected
            $('#user-email').val(''); // Clear the hidden input
            $('#submit-btn').prop('disabled', true); // Disable the submit button
        } else {
            $('#user-email').val(selectedUserEmail); // Set the hidden input with the selected email
            console.log('User selected:', selectedUserEmail);
            validateForm(); // Validate form after selecting user
        }
    });

    // Ensure submit button is enabled if user is selected via change and keyup
    $('#user-dropdown').on('change', validateForm);
    $('#user-search').on('keyup', validateForm);
    ...
    function validateForm() {
        ...
        console.log('Validating form:', 'Role:', role, 'User Email:', userEmail);
    ...
    $('#post-form').on('submit', function(e) {
        ...
        console.log('Form submit attempt:', 'Role:', role, 'User Email:', userEmail);
